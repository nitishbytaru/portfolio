...
import {
  Github, Star, GitFork, Users, Eye, Code2, Calendar,
  Activity, TrendingUp, Award, ExternalLink, Clock,
  Zap, BarChart3, Globe, Sparkles, GitPullRequest,
  FolderGit2, Rocket, Trophy, Target, Layers,
} from "lucide-react";
...
import {
  ...
  fetchRepoLanguages, computeAnalytics, formatEventType, timeAgo,
...
const LanguageDonutChart = ({ languages, isVisible }) => {
  ...
  // Filter languages with valid percentage and sort them
  const validLangs = useMemo(() => {
    return languages.filter(lang => parseFloat(lang.percentage) > 0);
  }, [languages]);

  // Compute accumulated percentages for correct rotations
  ...
  // Find the language to display in center. Hovered takes priority, fallback to top language.
    ...
    <div className="relative h-40 w-full">
      <svg viewBox="0 0 400 160" className="w-full h-full" preserveAspectRatio="none">
        {timeline.map((item, i) => {
          ...
          const x = i * (400 / timeline.length) + 10;
          const w = 400 / timeline.length - 20;
          return (
            <g key={item.year}>
              <rect
                x={x} y={140 - (isVisible ? barH : 0)} width={w} rx="6"
                height={isVisible ? barH : 0}
                fill="var(--primary)" opacity="0.7"
                className="transition-all duration-1000 ease-out"
                style={{ transitionDelay: `${i * 150}ms` }}
              />
              <text x={x + w / 2} y="156" textAnchor="middle" fontSize="11"
                fill="var(--text-secondary)" fontFamily="Inter">
                {item.year}
              </text>
              {isVisible && (
                <text x={x + w / 2} y={135 - barH} textAnchor="middle" fontSize="12"
...
/* ── Repo Card ─────────────────────────────────────── */
const RepoCard = ({ repo, index, isVisible }) => (
  <GlassCard
...
export default function GitHubAnalytics() {
  ...
  return (
    <SectionShell id="analytics">
      ...
      {/* ── ROW 2: Languages + Insights ──────────── */}
      <div className="grid gap-4 md:gap-6 md:grid-cols-5 mb-6">
        {/* Language Distribution — spans 3 */}
